# Playwright Gateway Design - AigenFlow 대체 아키텍처
...
AigenFlow(Electron 기반 데스크톱 앱)는 브라우저 세션 쿠키를 import하여 AI 서비스에 접근하는 방식을 사용한다.
...
AigenFlow를 제거하고, Playwright의 **persistent browser context**를 사용하여 AI 서비스에 직접 접근한다.
...
```
[기존 AigenFlow 방식]
CLI → HTTP 요청 → AigenFlow (Electron 앱, 별도 프로세스) → 내장 브라우저 → AI
...
```
최초 설정: aigenflow setup    → 브라우저 창 열림 → 각 AI에 로그인 → 프로필 저장
일반 사용: aigenflow run ...  → 저장된 프로필로 headless 실행 → 자동 동작
세션 만료: aigenflow relogin  → 만료된 서비스만 브라우저 창 열림 → 재로그인
```
...
셀렉터를 외부 설정 파일(`~/.aigenflow/selectors.yaml`)로 분리하여 코드 수정 없이 업데이트 가능.
...
| 항목 | 기존 (AigenFlow) | 변경 (Playwright) |
|------|---------------|-------------------|
| AI 게이트웨이 | AigenFlow Electron 앱 (별도 프로세스) | Playwright 내장 (파이프라인 내부) |
| 의존성 | aiohttp → localhost:3210 | playwright-python → 직접 브라우저 제어 |
| 세션 관리 | 수동 쿠키 import | 영구 프로필 + 자동 복구 체인 |
| 시스템 요구사항 | AigenFlow 사전 설치 필요 | Playwright 브라우저 자동 설치 |
...
| AigenFlow v3.0.0 (외부 앱) | playwright install chromium |
